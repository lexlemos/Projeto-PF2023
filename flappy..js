...
const move_floor = (execution) => (state) => {
    if(state.floor.x <= -state.floor.width) {
        return merge(state.floor)({x: 0})
    }
    return merge(state.floor)({x: state.floor.x - 0.25*execution.dt})
}

const move_background = (execution) => (state) => {
    if(state.background.x <= -state.background.width) {
        return merge(state.background)({x: 0})
    }
    return merge(state.background)({x: state.background.x - 0.25*execution.dt})
}
